...
def main():
    ...
    with torch.no_grad():
        ...
        for batch_dict in metric_logger.log_every(test_loader, args.batch_size, header):
            ...
            pkl_file = open(os.path.join(output_dir, f'{pretrained_model}_{args.batch_size}_masked_token_results.pkl'), 'wb')
            pickle.dump(mask_res_collector, pkl_file)
            pkl_file.close()

            with open(os.path.join(output_dir, f'rtx8000_{pretrained_model}_{args.batch_size}_flickr_results.pkl'), 'wb') as f:
            ...
            time_df.to_csv(os.path.join(output_dir, f'{args.pretrained_model}_{args.batch_size}_eval_time.csv'))
            i+=1
